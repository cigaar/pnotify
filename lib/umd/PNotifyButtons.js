(function (global, factory) {
	...
	var methods = {
		initModule: function initModule(options) {
			...
			this.get('_notice').observe('hide', function (hide) {
				...
				// make it again.
				var icon = hide ? _this.get('classes').pinUp : _this.get('classes').pinDown;
	...
	assign(assign(PNotifyButtons.prototype, methods), {
		destroy: destroy,
		get: get,
		fire: fire,
		observe: observe,
	...
	PNotifyButtons.prototype._recompute = function _recompute(changed, state) {
		if (changed.sticker || changed._notice) {
			if (this._differs(state._showSticker, state._showSticker = _showSticker(state.sticker, state._notice))) changed._showSticker = true;
		}

		if (changed.closer || changed._notice) {
			if (this._differs(state._showCloser, state._showCloser = _showCloser(state.closer, state._notice))) changed._showCloser = true;
		}
