(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define('PNotifyDesktop', ['exports', "PNotify"], factory);
    ...
    global.PNotifyDesktop = mod.exports;
  }
  ...
  var _notify = function notify(title, options, onclick, onclose) {
    ...
    if ('Notification' in window) {
      ...
      _notify = function notify(title, options, onclick, onclose) {
        // Gecko < 22
        var notice = navigator.mozNotification.createNotification(title, options.body, undefined.get('icon')).show();
        notice.onclick = onclick;
        notice.onclose = onclose;
        return notice;
      };
      ...
      _notify = function notify(title, options, onclick, onclose) {
        var notice = window.webkitNotifications.createNotification(undefined.get('icon'), title, options.body);
        notice.onclick = onclick;
        notice.onclose = onclose;
        return notice;
      };
  ...
  function data() {
    ...
    }, _PNotify2.default.modules.Desktop.defaults);
  };
  ...
  var methods = {
    initModule: function initModule(options) {
      ...
      notice.observe('_animatingClass', function (value) {
        if (value === '' || permission !== 0 && _this.get('fallback') || !_this.get('desktop')) {
          return;
        }
      ...
      notice.observe('animation', function (value, oldValue) {
        if (oldValue === undefined || value !== 'none' || oldValue === 'none' && value !== _this.get('_oldAnimation')) {
          _this.set({ '_oldAnimation': value });
        }
      });
      ...
      permission = _PNotify2.default.modules.Desktop.checkPermission();
      if (permission !== 0) {
    ...
    beforeOpen: function beforeOpen() {
      if (this.get('desktop') && permission !== 0) {
        _PNotify2.default.modules.Desktop.permission();
      }
      if (permission !== 0 && this.get('fallback') || !this.get('desktop')) {
        return;
      }
  ...
  function setup(Component) {
    ...
    _PNotify2.default.modules.Desktop = Component;
  };
  ...
  assign(assign(PNotifyDesktop.prototype, methods), {
    destroy: destroy,
    get: get,
    fire: fire,
    observe: observe,
  ...
  exports.default = PNotifyDesktop;
});