(function (global, factory) {
	...
	var methods = {
		...
		doSomething: function doSomething() {
			...
			var timer = setInterval(function () {
				...
				notice.refs.elem.style.transform = 'rotate(' + curAngle + 'deg)';
			}, 20);
		},
	...
	function create_main_fragment(component, state) {
		...
		var if_block = state.putThing && create_if_block(component, state);
	...
	function create_if_block(component, state) {
		...
		return {
			...
			h: function hydrate() {
				i.className = i_class_value = "" + state._notice.get('_icons').athing + " svelte-1qy4b0e";
			...
			p: function update(changed, state) {
				if (changed._notice && i_class_value !== (i_class_value = "" + state._notice.get('_icons').athing + " svelte-1qy4b0e")) {
	...
	assign(assign(PNotifyReference.prototype, methods), {
		destroy: destroy,
		get: get,
		fire: fire,
		observe: observe,
